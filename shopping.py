...
def load_data(filename):
    ...
    with open(filename) as f:
        ...
        ints=["Administrative", "Informational", "ProductRelated",
        ...
        for i,line in enumerate(f):
            ...
            for x in range(17):
                if header[x] in ints:
                    temp.append(int(row[x]))
                elif header[x] in floats:
                    temp.append(float(row[x]))
                elif header[x]=="Month":
                    temp.append(Month[row[x]])
                elif header[x]=="VisitorType":
                    try:
                        temp.append(VisitorType[row[x]])
                    except:
                        temp.append(0)
                elif header[x]=="Weekend":
                    temp.append(Weekend[row[x]])
            evidence.append(temp)
            labels.append(Revenue[row[x]])
        t=(evidence,labels)
        return(t)
...
def evaluate(labels, predictions):
    ...
    return (correct_pos/true_pos , correct_neg/true_neg)
